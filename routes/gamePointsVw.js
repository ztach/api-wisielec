let express = require('express');
let gamePoints = require('../models').gamePointsVw;
let router = express.Router();
let Sequelize=require('sequelize');
const Op = Sequelize.Op;

// middleware
var checkIDInput = function (req, res, next) {  
    if(isNaN(req.params.id)) {
        res.status(400).json('Invalid ID supplied');
    } else {
        next();
    }
};

var checkIDExist = function (req, res, next) {  
   gamePoints.count({ where: { id: req.params.id } }).then(count => {
        if (count != 0) {
            next();
        } else {
            res.status(400).json('gamePoints not found');
        }
    }); 
};

router.get('/', function(req, res){
    gamePoints.findAll().then(ur => {
        res.status(200).json(ur);
    });
});

//http://localhost:3100/gamePoints/login?login=osoba

router.get('/login', function(req, res){
    const x = req.query.login
    let mlike = {login: {[Op.like]:   '%' + x + '%' }} ;
    gamePoints.findAll({where:mlike})
    .then(item => res.status(200).json(item))
});


//http://localhost:3100/gamePoints/rola?rola=rola

router.get('/rola', function(req, res){
    const x = req.query.rola
    let mlike = {rola: {[Op.like]:   '%' + x + '%' }} ;
    gamePoints.findAll({where:mlike})
    .then(item => res.status(200).json(item))
});


//http://localhost:3100/gamePoints/rola/:rola_id

router.get('/rola/:rola_id', function(req, res){
    const x = req.params.rola_id;
    let mlike = {rola: {[Op.eq]: x }} ;
    gamePoints.findAll({where:mlike})
    .then(item => res.status(200).json(item))
});

//http://localhost:3100/gamePoints/user/:user_id
router.get('/user/:user_id', function(req, res){
    const x = req.params.user_id;
    console.log(x);
    let isUser = {user_id: {[Op.eq]: x }} ;
    gamePoints.findAll({where:isUser})
    .then(item => res.status(200).json(item))
});



router.get('/:id', [checkIDInput, checkIDExist], function(req, res){
    gamePoints.findByPk(req.params.id).then(item => {
        res.status(200).json(item);
    });
});


module.exports = router;